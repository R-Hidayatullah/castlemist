...
static void process_entry(const std::string& dat_path, const MftData& e, uint32_t base_id,
    ...
    if (e.compression_flag != 0 && stripped.size() >= 8) {
        ...
        try {
            final = castlemist::cmp::decompress_method0(std::span<const uint8_t>(stripped).subspan(8), usize);
...
static void init_schema(sqlite3* db) {
    ...
    // EXISTS leaves such a table alone, so without this the writer's INSERT fails
    // to PREPARE and every row is silently dropped while the run still reports
    // "processed N" -- which is how a re-index can quietly delete entries.
    ...
    sqlite3_finalize(st);
    if (!has_usize) {
        std::fprintf(stderr, "migrating: adding entries.uncompressed_size\n");
        exec(db, "ALTER TABLE entries ADD COLUMN uncompressed_size INTEGER;"
                 "UPDATE entries SET uncompressed_size = size_final;");
    }
}
...
int main(int argc, char** argv) {
    ...
    std::thread writer([&]() {
        ...
        // A failed prepare leaves the stmt null and every bind/step below a silent
        // no-op -- the run would report "processed N" while writing nothing. Abort
        // instead: a half-written index is worse than no run at all.
        auto prep = [&](const char* sql, sqlite3_stmt** out) {
            if (sqlite3_prepare_v2(db, sql, -1, out, nullptr) != SQLITE_OK) {
                std::fprintf(stderr, "FATAL: prepare failed: %s\n  sql: %s\n", sqlite3_errmsg(db), sql);
                std::exit(2);
            }
        };
        ...
        while (queue.pop(r)) {
            ...
            // compressed entry (99.4% of a retail dat), so it can't be used for this.
