...
static MatGPU build_material(Gw2Dat& dat, const nlohmann::json& tpl, const castlemist::model::Material& mat,
    ...
    castlemist::model::AmatSet set=castlemist::model::Extractor(amat,tpl).extractAmat();
    ...
    for(auto& b:set.shaders[psIdx].samplers){
        ...
        try{ std::vector<uint8_t> tx=decomp(dat,tb-1); if(tx.size()>=4&&tx[0]=='C')tx[0]='A';
             castlemist::atex::Texture t=castlemist::atex::parse(tx.data(),tx.size()); castlemist::atex::Image im=castlemist::atex::decode(t,0);
...
int main(int argc,char**argv){
    ...
    castlemist::model::Model model=castlemist::model::Extractor(modl,tpl).extract();
    if(model.meshes.empty()) die("no meshes");

    // combined VB/IB + submeshes
    std::vector<castlemist::model::Vertex> V; std::vector<uint32_t> I;
    ...
    { D3D11_BUFFER_DESC bd{}; D3D11_SUBRESOURCE_DATA sd{}; bd.Usage=D3D11_USAGE_IMMUTABLE;bd.BindFlags=D3D11_BIND_VERTEX_BUFFER;
      bd.ByteWidth=(UINT)(V.size()*sizeof(castlemist::model::Vertex)); sd.pSysMem=V.data(); ID3D11Device_CreateBuffer(g_dev,&bd,&sd,&vb);
    ...
    UINT vstride=sizeof(castlemist::model::Vertex),voff=0;
