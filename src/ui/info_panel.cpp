...
#include "castlemist/native/gw2model.hpp" // castlemist::model::describeFvf
#include "castlemist/db/index_db.h"   // pre-computed sizes/type from the gw2index SQLite
...
void append_model_info(std::wstring& text, const ExtractedEntry& entry) {
    ...
    if (m.hasAnimation) {
        ...
        text += buf;
        int shown = 0;
        for (const castlemist::granny::Anim& c : m.animClips) {
    ...
    for (size_t i = 0; i < m.meshes.size(); ++i) {
        ...
        int stride = 0;
        std::vector<std::string> comps = castlemist::model::describeFvf(mesh.fvf, &stride);
        swprintf(buf, 512,
...
void show_entry_info(HWND panel, const Gw2Dat& data_gw2, uint32_t mft_index, const ExtractedEntry& entry) {
    ...
    case PreviewKind::Audio: kind_name = L"Audio"; break;
    case PreviewKind::Content: kind_name = L"Content datastore (cntc)"; break;
    case PreviewKind::Video: kind_name = L"Video (Bink)"; break;
    case PreviewKind::Cinematic: kind_name = L"Cinematic (CINP)"; break;
    default: break;
    }
    ...
    // Bink cinematic: the stream facts, straight from its header.
    if (entry.kind == PreviewKind::Video && entry.video.ok) {
        const VideoMeta& v = entry.video;
        wchar_t vb[512];
        std::wstring codec(v.codec.begin(), v.codec.end());
        swprintf(vb, 512,
                 L"\r\n--- Video ---\r\n"
                 L"Codec: %ls\r\n"
                 L"Resolution: %u x %u\r\n"
                 ...
                 L"Audio tracks: %u\r\n"
                 L"Alpha plane: %ls\r\n",
                 codec.c_str(), v.width, v.height, v.frames,
                 v.fps_den ? static_cast<double>(v.fps_num) / v.fps_den : 0.0, v.fps_num, v.fps_den,
                 v.seconds, v.tracks, v.has_alpha ? L"yes" : L"no");
        text += vb;
    }

    // Pre-computed truth from the gw2index DB (baseId == mft_index + 1), when an
    // index is open: the DECOMPRESSED size and verified compression state are known
    // without decompressing anything, and they also cover entries whose preview
    // failed to build (where entry.decompressed is empty).
    if (castlemist::db::is_open()) {
        castlemist::db::EntryInfo ie = castlemist::db::lookup(mft_index + 1);
        if (ie.found) {
            wchar_t ib[512];
            std::wstring ty(ie.type.begin(), ie.type.end());
            std::wstring co(ie.container.begin(), ie.container.end());
            swprintf(ib, 512,
                     L"\r\n--- Index (gw2index DB) ---\r\n"
                     L"Indexed type: %ls%ls%ls\r\n"
                     L"On-disk size: %lld bytes\r\n"
                     L"Stripped (no CRC): %lld bytes\r\n"
                     L"Uncompressed size: %lld bytes\r\n"
                     L"Compression: flag %d, actually compressed: %ls\r\n",
                     ty.empty() ? L"?" : ty.c_str(),
                     co.empty() ? L"" : L" / ", co.c_str(),
                     ie.size, ie.size_stripped, ie.size_final,
                     ie.comp_flag, ie.actually_compressed ? L"yes" : L"NO (mis-flagged)");
            text += ib;
            if (!ie.chunks.empty()) {
                text += L"Chunks: ";
                for (size_t i = 0; i < ie.chunks.size(); ++i) {
                    if (i) text += L", ";
                    text += std::wstring(ie.chunks[i].begin(), ie.chunks[i].end());
                }
                text += L"\r\n";
            }
            if (!ie.error.empty())
                text += L"Index error: " + std::wstring(ie.error.begin(), ie.error.end()) + L"\r\n";
        }
    }
