...
ParsedNodePtr BinaryParser::gwString(const std::string& kind, const std::string& nodeName) {
    ...
    if (kind == "filename") {
        // Byte-exact port of the game's fileName->fileId decoder
        // sub_140DAB4A0 (Arena\Services\File3\FileArchive.cpp:702), verified in
        // IDA: words are low, high, third, fourth; the id is
        //   0xFF00*high + low - 0xFF00FF
        // gated by a validity check (low/high must be >= 0x100; a non-zero
        // third must itself be >= 0x100 and fourth must be 0). An invalid ref
        // yields fileId 0 in-game, so we report the same rather than a bogus id.
        uint16_t lo     = inBounds(t + 1, 1) ? readAt<uint16_t>(t) : 0;
        uint16_t hi     = inBounds(t + 3, 1) ? readAt<uint16_t>(t + 2) : 0;
        uint16_t third  = inBounds(t + 5, 1) ? readAt<uint16_t>(t + 4) : 0;
        uint16_t fourth = inBounds(t + 7, 1) ? readAt<uint16_t>(t + 6) : 0;
        bool invalid = lo < 0x100 || hi < 0x100 ||
                       (third != 0 && (third < 0x100 || fourth != 0));
        if (invalid) {
            node->valueString = "fileId=0 (invalid)";
        } else {
            long id = 0xFF00L * (long)hi + (long)lo - 0xFF00FF;
            node->valueString = "fileId=" + std::to_string(id);
            if (third != 0) node->valueString += " subId=" + std::to_string(third);
        }
