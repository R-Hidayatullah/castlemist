...
std::vector<castlemist::strs::Record> records(const uint8_t* data, size_t size, long long baseTextId) {
    std::vector<castlemist::strs::Record> recs = castlemist::strs::records(data, size);
    if (size < 4 || std::memcmp(data, "strs", 4) != 0) return recs;
    // Walk the payloads again in lockstep so packed records can be decrypted.
    size_t off = 4;
    for (auto& r : recs) {
        if (off + 6 > size) break;
        const uint8_t* payload = data + off + 6;
        size_t payload_len = (r.recLen >= 6) ? (size_t)r.recLen - 6 : 0;
        r.textId = (baseTextId >= 0) ? baseTextId + r.index : -1;
        if (r.packed) {
            auto it = (r.textId >= 0) ? g_keys.find((uint32_t)r.textId) : g_keys.end();
            if (it != g_keys.end()) {
                std::vector<uint8_t> buf(payload, payload + payload_len);
                uint8_t key20[20];
                expand_key(it->second, key20);
                rc4(key20, buf);
                r.text = bitunpack(buf.data(), buf.size(), r.baseChar, r.rangeBits);
                r.locked = false;
            }
        }
        off += r.recLen;
...
uint32_t fileid_for_textid(long long textId) {
    if (textId < 0 || g_textbase.empty()) return 0;
    uint32_t best = 0; long long bestBase = -1;
    for (const auto& kv : g_textbase) {
        if (kv.second <= textId && kv.second > bestBase) { bestBase = kv.second; best = kv.first; }
    }
    // Guard against picking a file the textId doesn't actually fall inside (files
    // hold ~1024 strings; reject if the gap is implausibly large).
    if (bestBase < 0 || textId - bestBase >= 4096) return 0;
    return best;
}

std::wstring decode_record_text(const uint8_t* data, size_t size, long long baseTextId, uint32_t target) {
    if (size < 4 || std::memcmp(data, "strs", 4) != 0) return {};
    size_t off = 4; uint32_t index = 0;
    while (off + 6 <= size) {
        ...
        if (index == target) {
            ...
            if (it != g_keys.end()) {
                ...
                expand_key(it->second, key20);
                rc4(key20, buf);
                return bitunpack(buf.data(), buf.size(), base_char, range_bits);
            }
            return {}; // packed, no key
        }
        off += rec_len; ++index;
    }
    return {};
