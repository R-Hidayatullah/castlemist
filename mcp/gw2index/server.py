...
Point it at a database with the GW2_INDEX_DB env var or the `db` argument on
any tool. The default is dumps/index/gw2_index.db in this repository.
"""
...
_ROOT = Path(__file__).resolve().parent.parent.parent  # mcp/gw2index/server.py
_DEFAULT_DB = os.environ.get(
    "GW2_INDEX_DB", str(_ROOT / "dumps" / "index" / "gw2_index.db")
)
...
def idx_query(type: Optional[str] = None, container: Optional[str] = None,
    ...
    with _conn(db) as c:
        ...
        rows = c.execute(
            # uncompressed_size = the real decompressed byte count (canonical name for
            # size_final). mft_usize is the MFT header's own field, which GW2 leaves 0
            # for every compressed entry -- kept only so that quirk stays visible.
            f"SELECT base_id,size,size_stripped,uncompressed_size,size_final,mft_usize,"
            f"comp_flag,actually_compressed,"
