...
/// block-row byte pitch, e.g. from castlemist::dds::DdsInfo::sys_mem_pitch.
...
/// Video streaming: creates (or reuses, when the size matches) a DYNAMIC BGRA8
/// texture of `width` x `height` and makes it the drawn surface, then uploads one
/// frame per update_video() call. Unlike set_texture -- which builds an IMMUTABLE
/// texture and would mean a fresh allocation 30 times a second -- this maps the
/// same resource every frame. Frames are top-down, `pitch` bytes per row.
