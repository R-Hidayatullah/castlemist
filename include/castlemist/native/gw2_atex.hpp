...
inline Texture parse(const uint8_t* data, size_t n) {
    ...
    {
        uint32_t v19; std::memcpy(&v19, data, 4);
        uint32_t v22 = (v19 & 4) ? ((4 * (v19 & 2)) | 0x10) : (4 * (v19 & 2));
        switch (v19) {
            case 0x43455441u: v22 |= CONT_ATEC; break; // "ATEC"
            case 0x50455441u: v22 |= CONT_ATEP; break; // "ATEP"
            case 0x55455441u: v22 |= CONT_ATEU; break; // "ATEU"
            case 0x54455441u: v22 |= CONT_ATET; break; // "ATET"
            default: break;                            // ATEX, ATTX
        }
...
enum class Premult { Auto, ForceOn, ForceOff };
...
inline Image decode(const Texture& t, int level = 0, Premult premult = Premult::Auto) {
    ...
    if (premult == Premult::ForceOn) unpremultiply_alpha(im);  // Auto/ForceOff: never
    return im;
}
