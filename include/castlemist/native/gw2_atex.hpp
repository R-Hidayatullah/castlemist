...
//   auto tex = castlemist::atex::parse(data, size);          // throws std::runtime_error
//   castlemist::atex::Image img = castlemist::atex::decode(tex, 0);   // mip 0 -> img.rgba (w*h*4)
//
...
namespace castlemist::atex
{
    ...
    inline int fourcc_to_enum(const uint8_t *f)
    {
        ...
        static const M m[] = {
            {{'D', 'X', 'T', '1'}, 22}, {{'D', 'X', 'T', '2'}, 23}, {{'D', 'X', 'T', '3'}, 24}, {{'D', 'X', 'T', '4'}, 25}, {{'D', 'X', 'T', '5'}, 26}, {{'D', 'X', 'T', 'A'}, 27}, {{'D', 'X', 'T', 'L'}, 28}, {{'D', 'X', 'T', 'N'}, 29}, {{'3', 'D', 'C', 'X'}, 30}, {{'B', 'C', '5', 'X'}, 31}, {{'B', 'C', '7', 'X'}, 32}};
        for (auto &e : m)
            if (!std::memcmp(f, e.c, 4))
                return e.e;
        return -1;
    }
    ...
    // Container-type bits derived from the *magic*, exactly as ImgAtex_Decode
    // (sub_140B83040) computes them and stores at decoder+0x4C. ATEX/ATTX get no extra
    // bit; ATEC/ATEP/ATEU/ATET each set one. NOTE: this word is engine metadata only --
    // it is provably write-only in the image pipeline (see unpremultiply_alpha comment),
    // so it must NOT be used to alter pixel decoding. The names below are the usual
    // community readings of the tag's *purpose*, not confirmed decode behaviour.
    enum ContainerFlag : uint32_t
    {
        CONT_ATEC = 0x01, // "C" container (often cubemap)
        CONT_ATEP = 0x02, // "P" container
        CONT_ATEU = 0x04, // "U" container (often UI)
        CONT_ATET = 0x20, // "T" container (often terrain)
    };
    ...
    struct Texture
    {
        ...
        std::string fmt_name;
        int width, height;
        uint32_t container_flags = 0; // see ContainerFlag
        std::vector<Mip> mips;
    };
    struct Image
    {
                            ...
                            std::vector<uint8_t> &surf, int &bpb_out)
    {
        ...
        // BC7 (fmt_enum 32) shares FORMAT_FLAGS 0xB1 with DXT2-5 for physical
        // block layout (2 planes, 16-byte block), but unlike them its plane B
        // is opaque raw BC7 bitstream (mode/partition/endpoints/indices), not
        // a BC1-style 565 color block. It must never take part in the BC1
        // constant-color / constant-fill reconstruction passes below.
        bool isBC7 = (fmt_enum == 32);
        ...
        if (do_terrain)
            for (int i = 0; i < nbBlocks; ++i)
                if (((1u << (i & 0x3F)) & BORDERS) || ((1u << (i >> 6)) & BORDERS))
                {
                    bs(bmA, i);
                    bs(bmB, i);
                }
        ...
        auto copy = [&](int dst, int nn)
        {
            size_t avail = 0;
            if (pos < payload_end)
                avail = std::min((size_t)nn, payload_end - pos);
            if (avail)
                std::memcpy(surf.data() + dst, buf + pos, avail);
            pos += nn;
        };
        if (has_A)
            for (int i = 0; i < nbBlocks; ++i)
        ...
        if (do_terrain)
            terrain_mirror(surf.data(), nbBlocks);
    }
    ...
    {
        if (n < 12)
            throw std::runtime_error("ATEX: too short");
        static const char *MAG[] = {"ATEX", "ATTX", "ATEC", "ATEP", "ATEU", "ATET"};
        bool ok = false;
        for (auto *m : MAG)
            if (!std::memcmp(data, m, 4))
                ok = true;
        if (!ok)
            throw std::runtime_error("ATEX: bad magic");
        int fe = fourcc_to_enum(data + 4);
        if (fe < 0)
            throw std::runtime_error("ATEX: unsupported fourCC");
        ...
        // container_flags: port of the v22 computation in sub_140B83040. v19&2/v19&4
        // are always 0 for the "AT.." family (first byte 'A'=0x41), so in practice this
        // reduces to: ATEX/ATTX -> 0, ATEC -> 1, ATEP -> 2, ATEU -> 4, ATET -> 0x20.
        {
            uint32_t v19;
            std::memcpy(&v19, data, 4);
            uint32_t v22 = (v19 & 4) ? ((4 * (v19 & 2)) | 0x10) : (4 * (v19 & 2));
            switch (v19)
            {
            case 0x43455441u:
                v22 |= CONT_ATEC;
    ...
    namespace detail
    {
        ...
        inline void bc3_alpha(const uint8_t *blk, int off, uint8_t out[16])
        {
            ...
            uint64_t bits = 0;
            for (int i = 0; i < 6; ++i)
                bits |= (uint64_t)blk[off + 2 + i] << (8 * i);
            for (int k = 0; k < 16; ++k)
                out[k] = (uint8_t)lut[(bits >> (3 * k)) & 7];
        }
        ...
        inline const Bc7Mode BC7_MODES[8] = {
            {3, 4, 0, 0, 4, 0, 1, 0, 3, 0}, {2, 6, 0, 0, 6, 0, 0, 1, 3, 0}, {3, 6, 0, 0, 5, 0, 0, 0, 2, 0}, {2, 6, 0, 0, 7, 0, 1, 0, 2, 0}, {1, 0, 2, 1, 5, 6, 0, 0, 2, 3}, {1, 0, 2, 0, 7, 8, 0, 0, 2, 2}, {1, 0, 0, 0, 7, 7, 1, 0, 4, 0}, {2, 6, 0, 0, 5, 5, 1, 0, 2, 0}};
        ...
        inline int bc7_weight(int bits, int i)
        {
            ...
            return bits == 2 ? w2[i] : bits == 3 ? w3[i]
                                                 : w4[i];
        }
        ...
        inline void decode_bc7_block(const uint8_t *blk, uint8_t out[16][4])
        {
            ...
            auto sc = [](int v, int bits)
            { v <<= (8 - bits); return v | (v >> bits); };
            for (int i = 0; i < ne; ++i)
            {
                r[i] = sc(r[i], cb);
                g[i] = sc(g[i], cb);
                b[i] = sc(b[i], cb);
                a[i] = ab ? sc(a[i], ab) : 255;
            }
            ...
            {
                for (int k = 0; k < 16; ++k)
                    parts[k] = (BC7_P2[part] >> k) & 1;
                anchors[1] = BC7_A2[part];
            }
            else
            {
                for (int k = 0; k < 16; ++k)
                    parts[k] = (BC7_P3[part] >> (2 * k)) & 3;
                anchors[1] = BC7_A3a[part];
                anchors[2] = BC7_A3b[part];
            }
    ...
    inline Image decode_surface(int fmt_enum, const uint8_t *surf, size_t /*surf_n*/, int W, int H)
    {
        ...
        for (int by = 0; by < bh; ++by)
            for (int bx = 0; bx < bw; ++bx)
            {
                ...
                case 27:
                { // DXTA -- single 8-byte plane, no BC/color plane at all (ImgAtex_Decode
                    // state 8's fillpass_selectPlanes skips DXTA: flags&0x210==0). It shares its
                    // RLE constant-fill pass (state 12/13) with DXT4/5's ALPHA plane -- format
                    // range 25..28 (DXT4,DXT5,DXTA,DXTL) -- which writes {a,a,0,0,0,0,0,0}, i.e.
                    // 2 endpoint bytes + 6 zeroed index bytes. It is never touched by the BC1
                    // constant-color fill (state 14/15), which is gated on flags&0x210 (0 for
                    // DXTA). So the block layout is bc3_alpha's 2-endpoint-byte + 6-index-byte,
                    // 8-level interpolated scheme -- NOT bc1_colors' 565/2-bit 4-level palette.
                    bc3_alpha(surf, bi * 8, al);
                    for (int k = 0; k < 16; ++k)
                    {
                        px[k][0] = px[k][1] = px[k][2] = al[k];
                        px[k][3] = 255;
                    }
                    place(im, bx, by, px);
                    break;
                ...
                { // DXTL
                    ...
                    for (int k = 0; k < 16; ++k)
                    {
                        px[k][0] = c[k][0] * al[k] / 255;
                        px[k][1] = c[k][1] * al[k] / 255;
                        px[k][2] = c[k][2] * al[k] / 255;
                        px[k][3] = 255;
                    }
                    place(im, bx, by, px);
                    break;
                ...
                { // DXTN/3DCX/BC5
                    int o = bi * 16;
                    bc3_alpha(surf, o, ch0);
                    bc3_alpha(surf, o + 8, ch1);
                    for (int k = 0; k < 16; ++k)
                    {
                ...
                case 32:
                { // BC7
                    decode_bc7_block(surf + bi * 16, px);
                    place(im, bx, by, px);
                    break;
                }
                default:
    ...
    // Divides RGB by A wherever A>0 (reverses premultiplied-alpha compositing).
    // NOTE ON PROVENANCE (updated after full RE of ImgAtex_Decode / sub_140B83040):
    // The container magic (ATEX/ATTX/ATEC/ATEP/ATEU/ATET) is turned into a flag word
    // (ATEX/ATTX=0, ATEC=1, ATEP=2, ATEU=4, ATET=0x20) and stored at decoder+0x4C, BUT
    // a full-binary trace shows that field is *write-only*: it is set once in the
    // decoder and reset to 0 in ImgDecode, and is NEVER read by the decoder, by
    // IGrImageLoadAlloc (GrImage.cpp), or by any of the 7 image loaders. Pixel decode
    // depends ONLY on (1) the second fourCC -> GR_FORMAT enum and (2) the per-mip flags
    // dword. => ATEP pixel data decodes byte-identically to ATEX; there is NO
    // premultiplied-alpha (or any other) transform tied to the container type.
    // This routine is therefore kept ONLY as an optional manual tool (Premult::ForceOn)
    // for experimentation; it is no longer applied automatically. The container tag is
    // engine metadata (texture-usage hint), not a decode instruction.
    inline void unpremultiply_alpha(Image &im)
    {
        for (size_t i = 0; i < im.rgba.size(); i += 4)
        {
            uint8_t a = im.rgba[i + 3];
            if (a == 0 || a == 255)
                continue;
            for (int c = 0; c < 3; ++c)
            {
                int v = im.rgba[i + c] * 255 / a;
                im.rgba[i + c] = (uint8_t)(v > 255 ? 255 : v);
            }
        }
    }

    // tri-state override so callers can force the behaviour either way for A/B testing.
    // Auto == OFF: the RE proved the container flag never influences pixel decoding, so
    // ATEP is decoded exactly like ATEX. Use ForceOn only to experiment manually.
    enum class Premult
    {
        Auto,
        ForceOn,
    ...
    inline Image decode(const Texture &t, int level = 0, Premult premult = Premult::Auto)
    {
        const Mip &m = t.mips.at(level);
        Image im = decode_surface(t.fmt_enum, m.surface.data(), m.surface.size(), m.width, m.height);
        if (premult == Premult::ForceOn)
            unpremultiply_alpha(im); // Auto/ForceOff: never
        return im;
    }
