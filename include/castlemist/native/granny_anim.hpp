...
// DaKeyframes32f decodes with keyframed==true (no knots), then parse() converts it
// into a degree-0 spline with uniform knots at i*TimeStep, so by sample() time every
// non-identity/non-constant curve is a plain (knots, controls, degree) spline.
struct Curve {
    int   fmt      = F_Identity;
    int   degree   = 0;
    int   dim      = 0;          // output components (3 pos / 4 ori / 9 scaleshear)
    bool  keyframed= false;      // transient: set by decodeCurve, cleared in parse()
...
inline Anim parse(const uint8_t* b, size_t n, int ptrSize = 4) {
    ...
    // DaKeyframes32f stores dense per-frame samples with NO knots (granny_curve.cpp:
    // FIND_KNOT_COUNT = ControlCount/Dimension). Granny plays it back as a degree-0
    // curve whose knots are implicit and uniform at i*TimeStep. Materialize those
    // knots so the unified sampler maps sample-time (seconds) -> frame correctly.
    // Without this the keyframe path treated `t` (seconds) as a frame index, so any
    // clip using this format sampled the wrong frame ("aneh"). All the quantized
    // formats GW2 normally uses already carry real knots and are unaffected.
    const float ts = (a.timeStep > 0.0f) ? a.timeStep : 1.0f;
    auto materializeKeyframed = [&](Curve& c) {
        if (!c.keyframed) return;
        int frames = c.dim ? (int)(c.controls.size() / c.dim) : 0;
        c.knots.clear();
        c.knots.reserve(frames);
        for (int i = 0; i < frames; ++i) c.knots.push_back((float)i * ts);
        c.degree    = 0;      // Granny keyframes are degree 0 (piecewise-constant).
        c.keyframed = false;  // now a normal degree-0 spline; sampled by time.
    };
    ...
    for (uint32_t g = 0; g < tgc && g < 64; ++g) {
        ...
        for (uint32_t i = 0; i < ttc; ++i) {
            ...
            materializeKeyframed(tr.ori);
            materializeKeyframed(tr.pos);
            materializeKeyframed(tr.sca);
...
inline void sample(const Curve& c, float t, float* out, int expectDim) {
    ...
    // Note: DaKeyframes32f is converted to a degree-0 spline (uniform knots at
    // i*TimeStep) in parse(), so it flows through the paths below like any curve.
    ...
    // CurveFindKnot: first knot strictly greater than t (unclamped == 0..KC). This is
    // the segment end; t lies in [knots[ub-1], knots[ub]).
    int ub = (int)(std::upper_bound(c.knots.begin(), c.knots.end(), t) - c.knots.begin());
    int ki = ub;                       // clamped copy used by the degree>=1 window
    if (ki >= KC) ki = KC - 1;
    if (ki < 0) ki = 0;

    if (d <= 0) { // degree 0: piecewise-constant, hold the control at the SEGMENT START
        // (controls[ub-1]); this matches the t->knots[ub-1] endpoint of the degree>=1
        // path. Using ki here held controls[ub] instead -> the whole curve read one
        // keyframe ahead (frame 0 already showed frame 1's value).
        int idx = ub - 1; if (idx < 0) idx = 0; if (idx >= KC) idx = KC - 1;
